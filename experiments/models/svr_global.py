...
from experiments.models._svr_utils import evaluate, train_svr
...
class SVRGlobalExperiment(BaseExperiment):
    ...
    def _run_chronological(self, df: pd.DataFrame, output_dir: Path) -> ExperimentResult:
        ...
        train_metrics = evaluate(model, train_df, GLOBAL_FEATURES, TARGET_COLUMN)
        val_metrics = evaluate(model, val_df, GLOBAL_FEATURES, TARGET_COLUMN)
    ...
    def _run_rolling_window(self, df: pd.DataFrame, output_dir: Path, mode: str) -> ExperimentResult:
        ...
        # Save per-window metrics for inspection
